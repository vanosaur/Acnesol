...
def run_product_node(state: PipelineState) -> PipelineState:
    ...
    # Pass severity so fallback products are relevant if search fails/times out
    state.recommended_products = search_skincare_products(
        query, max_results=3, severity=base_severity
    )
    return state
